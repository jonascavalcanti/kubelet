...
export DOCKER_NET=`cat /run/flannel/subnet.env | grep FLANNEL_SUBNET | awk -F"=" '{print $2}'`
export DOCKER_MTU=`cat /run/flannel/subnet.env | grep FLANNEL_MTU | awk -F"=" '{print $2}'`


#START DOCKER
/usr/bin/dockerd --bip=$DOCKER_NET --mtu=$DOCKER_MTU -H tcp://0.0.0.0:2376 -H unix:///var/run/docker.sock
...
# --logtostderr=true \
# --v=2 \
# --master=http://cersei.funceme.br:8080
